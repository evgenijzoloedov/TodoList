...
btn.addEventListener('click',()=>{
    if (!inputtext.value){
        ...
        task.innerHTML=`
                <div class="cheakboxtitle">
                    <input class="cheakbox" type="checkbox" name="" id="" >
                    <span class="title">${text}</span>
                </div>
                <button class="remove" data-type="remove">remove</button>
        `
        inputtext.value=''
        tasks__todo.append(task)
        ...
        const cheakboxes=document.querySelectorAll('.cheakbox')
        cheakboxes.forEach(cheakbox=>{
            cheakbox.addEventListener('click', e=>{
                if (!!e.target.checked){
                    const fordel=e.target.closest('.tasks')
                    tasks__done.append(fordel)
                    number__todo.textContent=tasks__todo.childNodes.length-1
                    number__done.textContent=tasks__done.childNodes.length-3
                }else{
                    const fordel=e.target.closest('.tasks')
                    tasks__todo.append(fordel)
                    number__todo.textContent=tasks__todo.childNodes.length-1
                    number__done.textContent=tasks__done.childNodes.length-3
                }
            })
